...
| **P1-3** | 实现 MVP API（§4.3，含物料快捷建档）+ 群组角色校验 | 自研 | 2～3 天 |
...
| GET | `/materials/search` | ALL | 搜索物料 |
| GET | `/materials/catalog` | ALL | 物料目录 + 各库位库存，供出入库页使用 |
...
> **说明**：MVP 由 FastAPI 在写库前校验库存（见 §4.4）。低并发不实现乐观锁。Bitable 自动化仅作备用，非主路径。
